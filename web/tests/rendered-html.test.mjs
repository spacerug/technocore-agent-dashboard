...
test("renders a fail-closed TCLK Deal Lab with private recovery gating", () => {
  ...
  const compatibility = readFileSync("app/lib/tclk-compat.ts", "utf8");
  const route = readFileSync("app/api/technocore/route.ts", "utf8");
  ...
  assert.match(dealLab, /SEPTEMBER 3 COMPATIBILITY GUARD/);
  assert.match(dealLab, /publishHeartbeat/);
  assert.match(dealLab, /Complete record authentication/);
  assert.match(protocol, /authenticateTclkRecord/);
  assert.match(compatibility, /Receipt claims .* verified terminal state is/);
  assert.match(compatibility, /Record DID signature does not verify/);
  assert.match(compatibility, /Refund window is already open; a new lock is forbidden/);
  assert.match(compatibility, /Receipt reference does not match/);
